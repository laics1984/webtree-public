...
<template>
  ...
  >
    ...
    <div class="wt-header-menu-toggle">
      <button
        type="button"
        class="wt-header-menu-toggle__button wt-ui-button wt-ui-menu-button wt-self-ink"
...
/* The close sits ON the sheet, so it takes the sheet's pair rather than painting
   its own — one rule that is right in both variants (the palette surface, and
   the dark overlay gradient). It replaces an `--overlay`-only `color: #ffffff`
   that left `.wt-ui-menu-button`'s opaque light background underneath, i.e.
   white on white on every light-palette site. */
.wt-mobile-menu-sheet__close {
  background: transparent;
  color: inherit;
  box-shadow: none;
